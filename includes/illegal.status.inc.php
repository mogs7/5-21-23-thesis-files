...
if (isset($_POST["Reviewed"])) {
    ...
    illegalreviewed($conn, $id, $licenseNum, $date_time, $status, $badgeID, $lastName, $firstName, $middleName, $birthday, $licensePlate, $regNum);
...
if (isset($_POST["Unaddressed"])) {
    ...
    if (emptyInputStatusUn($licensePlate) !== false) {
        header("Location: ../illegallane.php?error=incompletedetails");
        exit();
    }
    illegalunaddressed($conn, $id, $status, $badgeID, $licensePlate, $date_time,);
    header("Location: ../illegallane.php?error=none");
}
